...
export function LiveCompetitionBanner() {
  const [visible] = useState(
    () => Date.now() < new Date(EVENT_END).getTime(),
  );
...
export function AddToCalendarButton() {
  ...
  return <button type="button" id="addToCal" onClick={downloadCalendar} className="inline-flex w-auto cursor-pointer items-center gap-2.5 rounded-[3px] border-2 border-transparent bg-[var(--yellow)] px-7 py-3.5 font-sans text-sm leading-[normal] font-bold text-white no-underline [transition:transform_.2s,box-shadow_.2s] hover:[transform:translateY(-3px)] hover:shadow-[0_6px_0_rgba(255,255,255,.24)] max-[640px]:w-full max-[640px]:justify-center max-[640px]:px-5 max-[640px]:py-3.5">
