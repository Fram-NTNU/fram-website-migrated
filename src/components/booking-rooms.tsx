...
// Faste UI-strenger i kortene, oversatt ved visning.
const labels = {
  nb: {
    showOnMap: "Vis på kart",
    back: "Tilbake",
    mapTitle: (room: string) => `Kart – ${room}`,
    seats: (capacity: string) => `${capacity} plasser`,
    explore: "Utforsk",
    meetingFeatures: ["Flatskjerm", "Lydplanke", "Stoler & bord"],
  },
  en: {
    showOnMap: "Show on map",
    back: "Back",
    mapTitle: (room: string) => `Map – ${room}`,
    seats: (capacity: string) => `${capacity} seats`,
    explore: "Explore",
    meetingFeatures: ["Flat screen", "Soundbar", "Chairs & tables"],
  },
} as const;

function MapHint({ lang }: { lang: Lang }) {
  return (
    <div className="mt-auto flex items-center gap-2 border-t border-[var(--line)] pt-[15px] text-[13.5px] font-semibold text-[var(--ink-soft)] [transition:color_.2s] group-hover:text-[var(--ink)]">
      <i className="ph ph-map-pin text-[16px] text-[var(--muted)]" />{labels[lang].showOnMap}
...
function MapBack({ maze, roomName, radius, onClose, mounted, scale, lang }: {
  maze: string; roomName: string; radius: string; onClose: () => void; mounted: boolean; scale?: number; lang: Lang;
}) {
  const t = labels[lang];
  return (
    <div className={`absolute inset-0 flex flex-col overflow-hidden ${radius} border border-[var(--line)] bg-[var(--card)] [backface-visibility:hidden] [transform:rotateY(180deg)]`}>
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-[var(--line)] px-4 py-2.5">
        ...
        <button type="button" onClick={onClose} className="inline-flex cursor-pointer items-center gap-1.5 rounded-full border border-solid border-[var(--line)] bg-[var(--bg-soft)] px-3 py-1.5 text-[12px] font-semibold text-[var(--ink)]">{t.back} <i className="ph ph-arrow-u-up-left text-[13px]" /></button>
      </div>
      ...
      <div className="relative flex-1 overflow-hidden bg-[var(--bg-soft)]">
        {mounted && (
          scale
            ? <iframe src={maze} className="absolute top-0 left-0 block border-0" style={{ width: `${100 / scale}%`, height: `${100 / scale}%`, transform: `scale(${scale})`, transformOrigin: "top left" }} loading="lazy" allow="geolocation" title={t.mapTitle(roomName)} />
            : <iframe src={maze} className="absolute inset-0 block h-full w-full border-0" loading="lazy" allow="geolocation" title={t.mapTitle(roomName)} />
        )}
      </div>
    </div>
  );
...
export function LargeRoom({ id, image, alt, capacity, title, description, features, maze, roomName, lang = "nb" }: {
  id?: string; image: string; alt: string; capacity: string; title: React.ReactNode; description: string; features: Feature[]; maze: string; roomName: string; lang?: Lang;
}) {
      ...
      <div className="relative h-full [transform-style:preserve-3d] [transition:transform_.7s_cubic-bezier(.4,0,.2,1)]" style={{ transform: flipped ? "rotateY(180deg)" : "rotateY(0deg)" }}>
        <article onClick={(e) => flipGuard(e) && open()} className="flex h-full cursor-pointer flex-col overflow-hidden rounded-[28px] border border-[var(--line)] bg-[var(--card)] [backface-visibility:hidden] [transition:transform_.25s,box-shadow_.25s] group-hover:[transform:translateY(-4px)] group-hover:shadow-[0_20px_50px_-25px_rgba(0,0,0,.18)]">
          ...
          <div className="flex flex-1 flex-col px-8 pt-7 pb-8">
            ...
            <div className="mb-6 flex flex-wrap gap-2"><span className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--bg-soft)] px-2.5 py-1.5 text-[12px] font-medium text-[var(--ink-soft)]"><i className="ph ph-users text-[14px]" />{labels[lang].seats(capacity)}</span>{features.map((feature) => <span key={feature.label} className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--bg-soft)] px-2.5 py-1.5 text-[12px] font-medium text-[var(--ink-soft)]"><i className={`${feature.icon} text-[14px]`} />{feature.label}</span>)}</div>
            <MapHint lang={lang} />
          </div>
        </article>
        <MapBack maze={maze} roomName={roomName} radius="rounded-[28px]" onClose={close} mounted={loaded} lang={lang} />
      </div>
    </div>
  );
}

export function MeetingRoom({ accent, image, alt, capacity, title, description, maze, roomName, lang = "nb" }: {
  accent: string; image: string; alt: string; capacity: string; title: React.ReactNode; description: string; maze: string; roomName: string; lang?: Lang;
}) {
  const { flipped, loaded, open, close } = useFlip();
  return (
      ...
      <div className="relative h-full [transform-style:preserve-3d] [transition:transform_.7s_cubic-bezier(.4,0,.2,1)]" style={{ transform: flipped ? "rotateY(180deg)" : "rotateY(0deg)" }}>
        <article onClick={(e) => flipGuard(e) && open()} className="flex h-full cursor-pointer flex-col overflow-hidden rounded-[22px] border border-[var(--line)] bg-[var(--card)] [backface-visibility:hidden] [transition:transform_.25s,border-color_.2s] group-hover:[transform:translateY(-3px)] group-hover:border-[var(--accent)]">
          ...
          <div className="flex flex-1 flex-col p-[22px]">
            ...
            <div className="mb-6 flex flex-wrap gap-1.5"><span className="rounded-md bg-[var(--bg-soft)] px-[9px] py-[5px] font-mono text-[10px] tracking-[.08em] text-[var(--ink-soft)] uppercase">{labels[lang].seats(capacity)}</span>{labels[lang].meetingFeatures.map((item) => <span key={item} className="rounded-md bg-[var(--bg-soft)] px-[9px] py-[5px] font-mono text-[10px] tracking-[.08em] text-[var(--ink-soft)] uppercase">{item}</span>)}</div>
            <MapHint lang={lang} />
          </div>
        </article>
        <MapBack maze={maze} roomName={roomName} radius="rounded-[22px]" onClose={close} mounted={loaded} scale={0.6} lang={lang} />
      </div>
    </div>
  );
}

export function SmallRoom({ id, image, alt, title, children, features, href, maze, logo, lang = "nb" }: {
  id?: string; image: string; alt: string; title: string; children: React.ReactNode; features: Feature[]; href?: string; maze?: string; logo?: string; lang?: Lang;
}) {
  const { flipped, loaded, open, close } = useFlip();
  const content = <>
    <div className="relative aspect-[16/11] overflow-hidden bg-[var(--bg-soft)]">
      ...
      {href && <span aria-hidden="true" className="absolute top-3.5 right-3.5 z-[2] rounded-full bg-[var(--ink)] px-[15px] py-[9px] text-[12.5px] leading-none font-semibold whitespace-nowrap text-white">{labels[lang].explore}</span>}
    </div>
  ...
  return (
    <div id={id} className="group h-full scroll-mt-[110px] [perspective:1600px]">
      <div className="relative h-full [transform-style:preserve-3d] [transition:transform_.7s_cubic-bezier(.4,0,.2,1)]" style={{ transform: flipped ? "rotateY(180deg)" : "rotateY(0deg)" }}>
        ...
        {maze && <MapBack maze={maze} roomName={title.replace(/\.$/, "")} radius="rounded-[22px]" onClose={close} mounted={loaded} lang={lang} />}
      </div>
    </div>
  );
}
