  ...
  return (
    <div className={`absolute inset-0 flex flex-col overflow-hidden ${radius} border border-[var(--line)] bg-[var(--card)] [backface-visibility:hidden] [transform:rotateY(180deg)]`}>
      <div className="flex shrink-0 items-center justify-between gap-2 border-b border-[var(--line)] px-4 py-2.5">
        ...
        <button type="button" onClick={onClose} className="inline-flex cursor-pointer items-center gap-1.5 rounded-[3px] border border-solid border-[var(--line)] bg-[var(--bg-soft)] px-3 py-1.5 text-[12px] font-semibold text-[var(--ink)]">Tilbake <i className="ph ph-arrow-u-up-left text-[13px]" /></button>
      </div>
  ...
  return (
    <div id={id} className="group h-full scroll-mt-[110px] [perspective:1600px]">
      <div className="relative h-full [transform-style:preserve-3d] [transition:transform_.7s_cubic-bezier(.4,0,.2,1)]" style={{ transform: flipped ? "rotateY(180deg)" : "rotateY(0deg)" }}>
        <article onClick={(e) => flipGuard(e) && open()} className="flex h-full cursor-pointer flex-col overflow-hidden rounded-[3px] border border-[var(--line)] bg-[var(--card)] [backface-visibility:hidden] [transition:transform_.25s,box-shadow_.25s] group-hover:[transform:translateY(-4px)] group-hover:shadow-[0_20px_50px_-25px_rgba(0,0,0,.18)]">
        ...
        <MapBack maze={maze} roomName={roomName} radius="rounded-[3px]" onClose={close} mounted={loaded} />
      </div>
    </div>
  );
}
...
export function MeetingRoom({ accent, image, alt, capacity, title, description, maze, roomName }: {
  ...
  return (
    <div style={{ "--accent": accent } as React.CSSProperties} className="group h-full [perspective:1600px]">
      <div className="relative h-full [transform-style:preserve-3d] [transition:transform_.7s_cubic-bezier(.4,0,.2,1)]" style={{ transform: flipped ? "rotateY(180deg)" : "rotateY(0deg)" }}>
        <article onClick={(e) => flipGuard(e) && open()} className="flex h-full cursor-pointer flex-col overflow-hidden rounded-[3px] border border-[var(--line)] bg-[var(--card)] [backface-visibility:hidden] [transition:transform_.25s,border-color_.2s] group-hover:[transform:translateY(-3px)] group-hover:border-[var(--accent)]">
          <div className="relative aspect-[16/10] shrink-0 overflow-hidden bg-[color-mix(in_oklab,var(--accent)_14%,var(--bg-soft))]">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              className="absolute inset-0 block h-full w-full object-cover [transition:transform_.5s_ease] group-hover:[transform:scale(1.05)]"
              width="1920" height="1280" loading="lazy" decoding="async" src={image} alt={alt}
            />
          </div>
        ...
        <MapBack maze={maze} roomName={roomName} radius="rounded-[3px]" onClose={close} mounted={loaded} scale={0.6} />
      </div>
    </div>
  );
}
...
export function SmallRoom({ id, image, alt, title, children, features, href, maze, logo }: {
  ...
  const content = <>
    <div className="relative aspect-[16/11] overflow-hidden bg-[var(--bg-soft)]">
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img
        className="block h-full w-full object-cover [transition:transform_.5s_ease] group-hover:[transform:scale(1.05)]"
        width="2200" height="1650" loading="lazy" decoding="async" src={image} alt={alt}
      />
      {logo && (
  ...
  const cardClasses = "group flex h-full flex-col overflow-hidden rounded-[3px] border border-[var(--line)] bg-[var(--card)] text-inherit no-underline [transition:transform_.25s,box-shadow_.25s,border-color_.2s]";
  ...
  return (
    <div id={id} className="group h-full scroll-mt-[110px] [perspective:1600px]">
      <div className="relative h-full [transform-style:preserve-3d] [transition:transform_.7s_cubic-bezier(.4,0,.2,1)]" style={{ transform: flipped ? "rotateY(180deg)" : "rotateY(0deg)" }}>
        ...
        {maze && <MapBack maze={maze} roomName={title.replace(/\.$/, "")} radius="rounded-[3px]" onClose={close} mounted={loaded} />}
      </div>
    </div>
  );
}
