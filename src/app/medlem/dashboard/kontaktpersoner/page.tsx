...
import { MemberContactsManager } from "@/components/member-contacts-manager";
import { MemberTopbar } from "@/components/member-topbar";
import { placeholderContacts } from "@/lib/member-contacts";
import { placeholderProfile } from "@/lib/member-profile";

// Under arbeid — skal ikke indekseres.
export const metadata: Metadata = {
  title: "Kontaktpersoner — FRAM NTNU",
  description: "Administrer organisasjonens kontaktpersoner på FRAM NTNU.",
  robots: { index: false, follow: false },
};

// TODO(auth/data): hent den innloggede orgens kontakter (og navn) her.
...
export default function ContactsPage() {
  return (
    <div className="flex min-h-screen flex-col bg-[#FBF7F0] font-sans text-[#1A1A1A] [--bg-soft:#F4EFE5] [--bg:#FBF7F0] [--blue:#2E86C1] [--card:#fff] [--ink-soft:#555] [--ink:#1A1A1A] [--line:#E9E2D3] [--muted:#8a8a8a] [--nav-accent:#E85A5A] [--orange:#E58A3A] [--yellow:#FDC82F]">
      ...
      <main className="mx-auto w-full max-w-[820px] flex-1 px-6 py-10 max-[520px]:px-4">
        <Link
        ...
        <div className="mb-8">
          ...
          <p className="mt-0 mb-0 max-w-[560px] text-[15px] leading-[1.6] text-[var(--ink-soft)]">
            Hold oversikt over hvem som representerer organisasjonen — leder, nestleder, IT-ansvarlig og andre roller.
          </p>
        </div>

        <MemberContactsManager initial={contacts} />
      </main>
    </div>
  );
}
