...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return <div className="mb-12 grid grid-cols-[1fr_auto] items-end gap-8 border-b border-current/15 pb-7"><h2 className="m-0 max-w-[16ch] text-[clamp(40px,4.6vw,64px)] leading-[1.02] font-extrabold tracking-[-.025em]">{children}</h2></div>;
}
  ...
  return <div className="min-h-screen bg-[var(--id-cream)] font-sans text-[var(--ink)] [scroll-behavior:smooth] [--bg-soft:#F2EDE3] [--bg:#FAF7F2] [--card:#fff] [--charcoal:#1E1E1E] [--id-blue:#4B9FD3] [--id-cream:#F5F9FC] [--id-peach:#FFB775] [--id-pink:#FF8FB8] [--id-teal:#3CBFAB] [--ink-soft:#555] [--ink:#1E1E1E] [--line:#E6E0D5] [--muted:#8A8A8A] [--nav-accent:#E85A5A] [--nav-bg:#F5F9FC] [--nav-border:rgba(30,30,30,.08)] [--red:#E85A5A] [--teal:#3CBFAB] [--yellow:#E85A5A]">
    ...
    <header className="relative flex h-[calc(100vh-82px-var(--id-banner-h,0px))] flex-col overflow-hidden max-[640px]:h-[calc(100svh-var(--id-banner-h,0px))]">
      ...
      <div className="absolute inset-x-0 bottom-0 z-[1] border-t border-white/12"><div className="mx-auto flex max-w-[1360px] items-end px-12 py-6 max-[900px]:px-5 max-[640px]:flex-col max-[640px]:items-start max-[640px]:gap-4 max-[640px]:pt-4 max-[640px]:pb-[18px] max-[520px]:px-4">
        ...
        <div className="flex items-center gap-3 max-[640px]:w-full max-[640px]:flex-col-reverse">
          <a href="#program" className="inline-flex items-center justify-center gap-2 rounded-[3px] border-2 border-white/45 px-7 py-3.5 text-sm font-bold text-white no-underline [transition:border-color_.2s,background_.2s] hover:border-white hover:bg-white/10 max-[640px]:w-full max-[640px]:px-5">Se program →</a>
          <AddToCalendarButton />
        </div>
      </div></div>
    </header>
    ...
    <section id="om" className="relative py-28 max-[640px]:py-12"><div className="mx-auto max-w-[1360px] px-12 max-[900px]:px-5 max-[520px]:px-4">
      <div className="grid grid-cols-[1.15fr_.85fr] items-start gap-[72px] max-[960px]:grid-cols-1 max-[640px]:gap-6">
        ...
        <aside className="aspect-square w-full overflow-hidden rounded-[3px] bg-black shadow-[0_20px_60px_rgba(0,0,0,.18)] max-[640px]:hidden"><DjVideo /></aside>
      </div>
    </div></section>
    ...
    <section id="galleri" className="relative border-t border-[var(--line)] bg-[var(--bg-soft)] py-28 max-[640px]:py-12"><div className="mx-auto max-w-[1360px] px-12 max-[900px]:px-5 max-[520px]:px-4">
      ...
      <div className="grid auto-rows-[160px] grid-cols-12 gap-3 max-[960px]:auto-rows-[140px] max-[960px]:grid-cols-2 max-[640px]:auto-rows-[180px] max-[640px]:gap-2">{gallery.map((image) => <div key={image.src} className={`relative overflow-hidden rounded-[3px] bg-[var(--bg-soft)] [transition:transform_.25s_ease] hover:[transform:scale(1.015)] ${image.className}`}>
        {/* eslint-disable-next-line @next/next/no-img-element */}
    ...
    <section id="video" className="relative bg-[linear-gradient(135deg,var(--id-blue)_0%,var(--id-teal)_100%)] py-28 text-[var(--bg)] max-[640px]:py-12"><div className="mx-auto max-w-[1360px] px-12 max-[900px]:px-5 max-[520px]:px-4">
      ...
      <div className="relative aspect-video overflow-hidden rounded-[3px] bg-black shadow-[0_24px_48px_rgba(0,0,0,.4)]"><iframe className="absolute inset-0 h-full w-full border-0" src="https://www.youtube-nocookie.com/embed/C31svE3VMlI?rel=0&modestbranding=1&playsinline=1" title="Innovasjonsdagene 2025 — Aftermovie" loading="lazy" referrerPolicy="strict-origin-when-cross-origin" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /></div>
    </div></section>
    ...
    <section id="program" className="relative scroll-mt-[82px] py-28 max-[640px]:py-12"><div className="mx-auto max-w-[1360px] px-12 max-[900px]:px-5 max-[520px]:px-4">
      ...
      <div className="grid grid-cols-2 gap-5 max-[960px]:grid-cols-1">{days.map((day) => <article key={day.number} className="flex flex-col overflow-hidden rounded-[3px] border border-black/8 bg-[var(--card)] max-[640px]:rounded-[3px]">
    ...
    <section id="miljoer" className="relative bg-[var(--charcoal)] pt-16 pb-px text-white max-[520px]:pb-0.5"><div className="mx-auto max-w-[1200px] px-12 max-[900px]:px-5 max-[520px]:px-4">
      ...
      <div className="rounded-[3px] border border-white/10 bg-white/[0.03] p-5 max-[520px]:p-3">
        <StandMap />
      </div>
      ...
      <div className="mt-8 flex flex-wrap items-center gap-3 pb-16"><Link href="/miljoer" className="inline-flex items-center gap-2.5 rounded-[3px] border-2 border-transparent bg-[var(--yellow)] px-7 py-3.5 text-sm font-semibold text-white no-underline [transition:transform_.2s,box-shadow_.2s] hover:[transform:translateY(-3px)] hover:shadow-[0_6px_0_rgba(255,255,255,.24)]">Se alle organisasjonene →</Link><a href="https://www.instagram.com/framntnu/" target="_blank" rel="noopener" className="inline-flex items-center rounded-[3px] border-2 border-white/30 px-7 py-3.5 text-sm font-semibold text-white/75 no-underline [transition:border-color_.2s,color_.2s] hover:border-white/70 hover:text-white">Følg @framntnu →</a></div>
    </div>
